...
for i in range(numOptim):
    ...
    ncpaOptim.endMode = 40  # wfc.getProperty("numModes")
...
plt.axvline(x=x, color="green", label=f"1 in 100 > {1e6*x:.0f}us")
x = np.percentile(sysLatency, 99.9)
plt.axvline(x=x, color="orange", label=f"1 in 1,000 > {1e6*x:.0f}us")
x = np.percentile(sysLatency, 99.99)
plt.axvline(x=x, color="red", label=f"1 in 10,000 > {1e6*x:.0f}us")
