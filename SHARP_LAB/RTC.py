...
# %% IMPORTS
# config = '/home/whetstone/pyRTC_backup/SHARP_LAB/config_SR.yaml'
config = "/home/whetstone/pyRTC_backup/SHARP_LAB/config.yaml"
N = np.random.randint(3000, 6000)

# %% Launch DM
wfc = hardwareLauncher("../pyRTC_backup/hardware/ALPAODM.py", config, N)
wfc.launch()

# %% Launch WFS
wfs = hardwareLauncher("../pyRTC_backup/hardware/ximeaWFS.py", config, N + 1)
wfs.launch()

# %% Launch slopes
slopes = hardwareLauncher("../pyRTC_backup/SlopesProcess.py", config, N + 2)
slopes.launch()

# %% Launch PSF Cam
psf = hardwareLauncher(
    "../pyRTC_backup/hardware/SpinnakerScienceCam.py", config, N + 10
)
psf.launch()

# %% Launch Loop Class
# loop = hardwareLauncher("../pyRTC_backup/Loop.py", config, N+4)
...
if RECALIBRATE == True:
    ...
    input("Sources Off?")
    wfs.run("takeDark")
    wfs.setProperty("darkFile", "/home/whetstone/pyRTC_backup/SHARP_LAB/calib/dark.npy")
    wfs.run("saveDark")
    time.sleep(1)
    psf.run("takeDark")
    psf.setProperty(
        "darkFile", "/home/whetstone/pyRTC_backup/SHARP_LAB/calib/psfDark.npy"
    )
    psf.run("saveDark")
    input("Sources On?")
    ...
    psf.setProperty(
        "modelFile", "/home/whetstone/pyRTC_backup/SHARP_LAB/calib/modelPSF_SH.npy"
    )
    ...
    loop.setProperty("IMFile", "/home/whetstone/pyRTC_backup/SHARP_LAB/calib/IM_SH.npy")
    wfc.run("flatten")
    loop.run("computeIM")
    loop.run("saveIM")
    wfc.run("flatten")
    ...
    # loop.setProperty("IMFile", "/home/whetstone/pyRTC_backup/SHARP_LAB/calib/IM_OL_docrime.npy")
    # wfc.run("flatten")
    # loop.run("computeIM")
    # loop.run("saveIM")
    # wfc.run("flatten")
...
# %% Compute CM
loop.setProperty("IMFile", "/home/whetstone/pyRTC_backup/SHARP_LAB/calib/IM_SH.npy")
# loop.setProperty("IMFile", "/home/whetstone/pyRTC_backup/SHARP_LAB/calib/OL_DOCRIME.npy")
# loop.setProperty("IMFile", "/home/whetstone/pyRTC_backup/SHARP_LAB/calib/OL_DOCRIME_CL_docrime.npy")
# loop.setProperty("IMFile", "/home/whetstone/pyRTC_backup/SHARP_LAB/calib/ESCAPE.npy")
...
a, b, c = (
    ...
    np.std(OLDOCRIME, axis=(0)),
    np.std(CLDOCRIME, axis=(0)),
)
...
assert psf.run("setGain", 19)

optuna.logging.set_verbosity(optuna.logging.DEBUG)

numOptim = 1
maxAMP = 0.001
amps = np.linspace(maxAMP, maxAMP / numOptim, numOptim)

for i in range(numOptim):
    ncpaOptim.resetStudy()
    psf.setProperty("integrationLength", 5)

    time.sleep(1)

    ncpaOptim.numReads = 3
    ncpaOptim.startMode = 0
    ncpaOptim.endMode = 85  # *(1+i) #wfc.getProperty("numModes")
    ncpaOptim.numSteps = 2500  # *(1+i)
    ...
    # wfc.run("saveShape")
    # slopes.run("takeRefSlopes")
    # slopes.setProperty("refSlopesFile", "/home/whetstone/pyRTC_backup/SHARP_LAB/calib/ref_SHWFS.npy")
    # slopes.run("saveRefSlopes")
...
aps_to_remove[: aps_to_remove.shape[0] // 2] |= aps_to_remove[
    aps_to_remove.shape[0] // 2 :
]
aps_to_remove[aps_to_remove.shape[0] // 2 :] |= aps_to_remove[
    : aps_to_remove.shape[0] // 2
]
...
numReads = 100
folder = "/home/whetstone/pyRTC_backup/SHARP_LAB/data/"
