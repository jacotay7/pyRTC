...
class ImageSHM:
        ...
        if self.areData:
            ...
            if self.gpuDevice is not None:
                ...
                if self.torchDtype is None:
                    ...
                    logging.log(
                        level=logging.WARNING,
                        msg=f"{self.name}: dtype {self.dtype} not supported for GPU SHM; defaulting to CPU mode.",
                    )
                    return
    ...
    def read(self, SAFE=True, GPU=False, RELEASE_GIL=True) -> NDArray:
        self.hold(RELEASE_GIL=RELEASE_GIL)
        return self.read_noblock(SAFE=SAFE, GPU=GPU)

    def read_timeout(self, timeout, SAFE=True, GPU=False, RELEASE_GIL=True) -> NDArray:
        self.hold(timeout=timeout, RELEASE_GIL=RELEASE_GIL)
        return self.read_noblock(SAFE=SAFE, GPU=GPU)

    def read_noblock(self, SAFE=True, GPU=False) -> NDArray:
    ...
    def listen(self):
        ...
        if requestType == "shutdown":
            ...
            try:
                ...
                logger.exception(
                    "Listener get request failed for %s", request.get("property")
                )
                self.write(self.BadMessage)
            ...
            try:
                ...
                logger.exception(
                    "Listener set request failed for %s", request.get("property")
                )
                self.write(self.BadMessage)
            ...
            try:
                ...
                logger.exception(
                    "Listener run request failed for %s", request.get("function")
                )
                self.write(self.BadMessage)
        else:
