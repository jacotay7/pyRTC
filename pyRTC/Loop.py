...
@jit(nopython=True, nogil=True, cache=True, fastmath=True)
def compCorrection(
    CM=np.array([[]], dtype=np.float32), slopes=np.array([], dtype=np.float32)
):
...
class Loop(pyRTCComponent):
    ...
    def __init__(self, conf) -> None:
        ...
        self.signalShm, self.signalShape, self.signalDType = initExistingShm(
            "signal", gpuDevice=self.gpuDevice
        )
        ...
        self.wfcShm, self.wfcShape, self.wfcDType = initExistingShm(
            "wfc", gpuDevice=self.gpuDevice
        )
        ...
        self.docrimeBuffer = np.zeros(
            (1 + self.delay, *tmp2.shape), dtype=self.wfcDType
        )
        ...
        self.controlLimits = setFromConfig(
            self.conf, "controlLimits", [-np.inf, np.inf]
        )
        self.integralLimits = setFromConfig(
            self.conf, "integralLimits", [-np.inf, np.inf]
        )
        self.absoluteLimits = setFromConfig(
            self.conf, "absoluteLimits", [-np.inf, np.inf]
        )
    ...
    def docrimeIM(self):
        ...
        for i in range(self.numItersIM):
            ...
            correction = (
                np.random.uniform(-self.pokeAmp, self.pokeAmp, correction.size)
    ...
    def computeCM(self):
        ...
        self.CM[: self.numActiveModes, :] = np.linalg.pinv(
            self.IM[:, : self.numActiveModes], rcond=0
        )
    ...
    def standardIntegratorPOL(self):
        ...
        newCorrection = self.updateCorrectionPOL(
            correction=currentCorrection, slopes=residual_slopes
        )
    ...
    def standardIntegrator(self):
        ...
        newCorrection = leakyIntegratorNumba(
            slopes,
            self.gCM,
    ...
    def pidIntegrator(self, slopes=None, correction=None):
        ...
        wfError = compCorrection(CM=self.CM, slopes=slopes)
        derivative = wfError - self.previousWfError

        # Apply low-pass filter to the derivative to reduce noise
        derivative = (
            self.derivativeFilter * derivative
            + (1 - self.derivativeFilter) * self.previousDerivative
        )
        ...
        isClipped = np.any(self.controlOutput == self.controlLimits[0]) or np.any(
            self.controlOutput == self.controlLimits[1]
        )
        ...
        controlOutput = (
            self.pGain * wfError + self.iGain * self.integral + self.dGain * derivative
        )
        ...
        newCorrection = (
            1 - self.leakyGain
        ) * correction - controlOutput  # Negative control direction is convention for pyRTC
    ...
    def linearExtrapolationPOL(self):
        ...
        # Update Command Vector c_n = g*CM*s_{POL} + (1 − g) c_{n-1}  https://arxiv.org/pdf/1903.12124.pdf Eq 3
        newCorrection = (1 - self.gain) * currentCorrection - np.dot(
            self.gCM, s_pol_pred
        )
    ...
    def sendToWfc(self, correction, slopes=None):
        ...
        if self.clDocrime and isinstance(slopes, np.ndarray):
            ...
            randShape = (
                np.random.uniform(-self.pokeAmp, self.pokeAmp, correction.size)
                .astype(self.docrimeBuffer[0].dtype)
                .reshape(self.docrimeBuffer[0].shape)
            )
    ...
    def solveDocrime(self):
        ...
        self.clDCIM = (self.docrimeCross / self.numItersDC) @ np.linalg.inv(
