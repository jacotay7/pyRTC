...
class _OOPAOWFCorrector(WavefrontCorrector):
    ...
    def __init__(self, correctorConf, tel, dm, layout=None) -> None:
        ...
        if layout is not None:
            ...
            self.setLayout(
                self.dm.validAct.reshape(
                    int(np.sqrt(numActuators)), int(np.sqrt(numActuators))
                )
            )
        self.setFlat(np.zeros(numActuators))
...
class OOPAOInterface:
    ...
    def __init__(self, conf, param=None) -> None:
        ...
        self.src = Source(optBand=param["sourceBand"], magnitude=param["magnitude"])
        ...
        self.wfsInterface = _OOPAOWFSensor(
            wfsConf, self.tel, self.ngs, self.atm, self.dm, self.wfs
        )
        self.dmInterface = _OOPAOWFCorrector(
            correctorConf, self.tel, self.dm, self.param["dm_layout"]
        )
        self.psfInterface = _OOPAOScienceCamera(
            scienceConf, self.tel_psf, self.src, self.atm, self.dm
        )
...
def _initializeDummyParameterFile():
    ...
    param["nSubaperture"] = (
        10  # number of PWFS subaperture along the telescope diameter
    )
    ...
    param["centralObstruction"] = (
        0.112  # central obstruction in percentage of the diameter
    )
    ...
    param["dm_layout"] = None

    # mis-registrations
    param["shiftX"] = (
        0  # shift X of the DM in pixel size units ( tel.D/tel.resolution )
    )
    param["shiftY"] = (
        0  # shift Y of the DM in pixel size units ( tel.D/tel.resolution )
    )
