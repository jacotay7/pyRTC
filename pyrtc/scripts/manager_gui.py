...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--mode",
        choices=["soft", "hard", "soft-rtc", "hard-rtc"],
        default=None,
        help="Optional manager mode override",
    )
    parser.add_argument(
        "--theme", choices=["dark", "light"], default="dark", help="Initial GUI theme"
    )
    parser.add_argument(
        "--refresh-ms",
        type=int,
        default=1000,
        help="Health and status refresh interval in milliseconds",
    )
...
def main(argv=None) -> int:
    ...
    logger = configure_logging_from_args(
        args, app_name="pyrtc-manager-gui", component_name="manager-gui"
    )
