...
class RealTimeView(QMainWindow):
    def __init__(self, shm_name, fps, static_vmin=None, static_vmax=None):
        ...
        self.metadata = ImageSHM(
            shm_name + "_meta", (ImageSHM.METADATA_SIZE,), np.float64
        )
    ...
    def update_view(self):
        ...
        if new_time > self.old_time:
            speed_fps = np.round(
                (new_count - self.old_count) / (new_time - self.old_time), 2
            )
...
if __name__ == "__main__":
    ...
    view = RealTimeView(shm_name, 30, static_vmin=static_vmin, static_vmax=static_vmax)
